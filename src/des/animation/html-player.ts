...
(function() {
  ...
  let PAYLOAD;
  try {
    PAYLOAD = JSON.parse(document.getElementById('anim-data').textContent);
  } catch (err) {
    document.body.innerHTML = '<p style="color:#b00020;padding:2rem;font-family:sans-serif">'
      + 'Failed to parse animation data: ' + (err && err.message ? err.message : String(err)) + '</p>';
    return;
  }
  const VARIANTS = Array.isArray(PAYLOAD.variants) && PAYLOAD.variants.length ? PAYLOAD.variants : null;
